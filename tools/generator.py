...
class LanguageTarget:
    ...
    def process_input_param(self, input):
        ...
        if self.is_array_input(input.name):
            ...
            if self.array_need_size():
                return f"{array_param}, int input_size"
            else:
                return array_param
        else:
    ...
    def process_attr_param(self, attr):
        ...
        if self.is_array_attr(attr) and self.array_need_size():
            return f"{attr_param}, int {attr.name}_size"
        else:
            return attr_param
...
class CAPISRC(LanguageTarget):
    ...
    def init_type_map(self):
        self.type_map = {
            ...
            # OpSchema.AttrType.TYPE_PROTOS: 'Error',
        }
...
class CSharpGenerator:
    ...
    def root_path(f: str) -> str:
        return os.path.join('..', 'CSharp', 'OrtKI', f)
    ...
    @staticmethod
    def class_decl():
        return f"""public partial class {CSharpGenerator.kernel_class()}\n"""
...
class CSharpDLLImport(LanguageTarget):
    ...
    def make_decl(self) -> str:
        dll_import = '[DllImport("libortki.so")]' + "\n" 
        decl = self.method_sign('private static extern')
...
class CSharpWrapper(CSharpDLLImport):
    ...
    def process_input_args(self, input):
        if self.is_array_input(input.name):
            return f"{input.name}.Select(x=>x.Handle).ToArray(), {input.name}.Length"
        else:
            return f"{input.name}.Handle"

    def process_attr_args(self, attr):
        if self.is_array_attr(attr):
            return f"{attr.name}, {attr.name}.Length"
        else:
            return f"{attr.name}"
    ...
    def wrap_impl(self) -> str:
        def process_attr(attr):
            v = attr.name
            if attr.type == OpSchema.AttrType.TENSOR:
                v += ".Handle"
            return v
        ...
        return f"""var _tensor = {self.interface_method_name()}({method_args(self)});
